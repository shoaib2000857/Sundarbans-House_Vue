  ...
  <div :data-theme="theme">
    ...
    <div id="preloader" v-if="loading" :style="{ opacity: preloaderOpacity }">
      <div class="preloader-inner">
        <div class="preloader-orbit-wrap">
          <div class="preloader-orbit-ring">
            <svg viewBox="0 0 220 220" xmlns="http://www.w3.org/2000/svg">
              <defs>
                <path id="orbitPath" d="M 110,110 m -88,0 a 88,88 0 1,1 176,0 a 88,88 0 1,1 -176,0" />
              </defs>
              <text>
        ...
        <div class="preloader-bar"><div class="preloader-fill" :style="{ width: fillWidth }"></div></div>
        <div class="preloader-text">Loading Sundarbans...</div>
      </div>
    </div>

    <!-- CUSTOM CURSOR -->
    <div id="cursor-dot" :style="dotStyle"></div>
    <div id="cursor-ring" :class="{ expanded: cursorExpanded }" :style="ringStyle"></div>
    ...
    <div class="search-overlay" :class="{ open: searchOpen }" @click.self="searchOpen = false">
      <div class="search-box">
        <div class="search-input-row">
          ...
          <input type="text" v-model="searchQuery" @input="filterSearch" placeholder="Search pages, resources, events..." ref="searchInputEl" />
        ...
        <div id="searchResults">
          ...
          <template v-else>
            <router-link v-for="p in filteredPages" :key="p.url" :to="p.url" class="search-result-item" @click="searchOpen = false">
              <div class="sri-icon">{{ p.icon }}</div>
              <div><div class="sri-title">{{ p.title }}</div><div class="sri-desc">{{ p.desc }}</div></div>
            </router-link>
          </template>
        </div>
      </div>
    ...
    <!-- BACK TO TOP -->
    <button id="backToTop" :class="{ visible: showBackToTop }" @click="scrollTop" aria-label="Back to top">
    ...
    <header class="navbar" :class="{ scrolled: navScrolled }" id="navbar">
      <div class="nav-container">
        ...
        <button class="menu-toggle" :class="{ active: menuOpen }" @click="menuOpen = !menuOpen" aria-label="Toggle menu">
        ...
        <nav class="nav-links" :class="{ open: menuOpen }">
          ...
          <router-link to="/meetups" class="nav-link" @click="menuOpen = false"><span>Meetups</span></router-link>
          <router-link to="/about" class="nav-link" @click="menuOpen = false"><span>About</span></router-link>
          <router-link to="/community" class="nav-link" @click="menuOpen = false"><span>Community</span></router-link>
          <router-link to="/members" class="nav-cta-btn" @click="menuOpen = false">
        ...
        <div class="nav-tools">
          <button class="nav-tool-btn" @click="openSearch" title="Search (Ctrl+K)">
          ...
          <div style="position:relative">
            ...
            <div class="notif-panel" :class="{ open: notifOpen }">
              ...
              <div class="notif-list">
                <div v-for="(n, i) in notifications" :key="i" class="notif-item" :class="{ unread: n.unread }" @click="markRead(i)">
          ...
          <button class="nav-tool-btn" @click="toggleTheme" title="Toggle theme">
...
// --- CURSOR ---
const dotX = ref(0), dotY = ref(0), ringX = ref(0), ringY = ref(0)
const cursorExpanded = ref(false)
let mX = 0, mY = 0, animFrame = null

function dotStyle() { return { left: dotX.value + 'px', top: dotY.value + 'px' } }
function ringStyle() { return { left: ringX.value + 'px', top: ringY.value + 'px' } }
const dotStyleC = computed(() => ({ left: dotX.value + 'px', top: dotY.value + 'px' }))
const ringStyleC = computed(() => ({ left: ringX.value + 'px', top: ringY.value + 'px' }))

function onMouseMove(e) {
  mX = e.clientX; mY = e.clientY
  dotX.value = mX; dotY.value = mY
}
function animRing() {
...
const pages = [
  ...
  { title: 'Study Corner', url: '/study', desc: 'Academic resources and materials', icon: '📚' },
  { title: 'Events', url: '/events', desc: 'Upcoming events, workshops and competitions', icon: '🎯' },
  { title: 'Gallery', url: '/gallery', desc: 'Photos and memories from our community', icon: '🖼️' },
  { title: 'Meetups', url: '/meetups', desc: 'City meetups across India', icon: '📍' },
  { title: 'About', url: '/about', desc: 'About Sundarbans House and our mission', icon: 'ℹ️' },
  { title: 'Community', url: '/community', desc: 'Tech, Cultural and Academic communities', icon: '🤝' },
  { title: 'Teams', url: '/teams', desc: 'Meet our leadership and team members', icon: '👥' },
  { title: 'Contact', url: '/contact', desc: 'Get in touch with us', icon: '📧' },
  { title: 'Social Media', url: '/social', desc: 'Follow us on social platforms', icon: '📱' },
  { title: 'Members Lounge', url: '/members', desc: 'Login to members area', icon: '🔐' },
]
...
function markRead(i) { notifications.value[i].unread = false }
...
function onClickOutside(e) {
  if (notifOpen.value && !e.target.closest('.notif-panel') && !e.target.closest('[data-notif]')) {
...
.nav-link.router-link-exact-active { color: var(--accent) !important; }
</style>
